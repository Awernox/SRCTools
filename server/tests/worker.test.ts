...
test("defaults to Run pro, Bhop pro and a two-second interval", () => {
  const config = loadConfig({
    DISCORD_WEBHOOK_URL: "https://discord.com/api/webhooks/123456789012345678/abcdefghijklmnopqrstuvwxyz",
    SPEEDRUN_API_KEY: "test-key",
  });
  assert.equal(config.checkIntervalMs, 2_000);
...
test("keeps two-second polling regardless of legacy interval variables", () => {
  const config = loadConfig({
    DISCORD_WEBHOOK_URL: "https://discord.com/api/webhooks/123456789012345678/abcdefghijklmnopqrstuvwxyz",
    SPEEDRUN_API_KEY: "test-key",
